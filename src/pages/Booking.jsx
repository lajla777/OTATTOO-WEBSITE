...
// --- Admin nedosegljivost: cel dan (termin === null) ali samo dopoldne/popoldne ---
const jeCelDanBlokiranAdmin = (nedosegljivi, dateStr) =>
  nedosegljivi.some(n => n.datum === dateStr && n.termin === null)

const jeTerminBlokiranAdmin = (nedosegljivi, dateStr, termin) =>
  jeCelDanBlokiranAdmin(nedosegljivi, dateStr) ||
  nedosegljivi.some(n => n.datum === dateStr && n.termin === termin)
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        <span>
          {value
            ? opcije.find(o => o.value === value)?.label
            : placeholder}
        </span>
...
function IzbiraCasa({
  ...
  const zasedeneUre = rezervacijeZaDan
    .map(r => r.cas)
    .filter(Boolean)

  const uraZasedena = ura => {
    // Admin je blokiral ta del dneva
    if (jeTerminBlokiranAdmin(nedosegljivi, datum, ura)) {
      return true
    }

    // Ta del dneva ima že rezervacijo
    if (zasedeneUre.includes(ura)) {
      return true
    }
  ...
  // VSE storitve imajo vedno možnost dopoldne / popoldne
  const ure = ['dopoldne', 'popoldne']
        ...
        {ure.map(ura => {
          ...
          return (
            ...
            >
              ...
              >
                {URA_LABELI[ura]} {disabled ? '— zasedeno' : ''}
              </p>
...
function Koledar({
  ...
  const getDanStatus = dateStr => {
    ...
    // Vikendi niso možni
    if (danVTednu === 0 || danVTednu === 6) {
      return 'ni_mozno'
    }

    // Pretekli datumi niso možni
    if (d < new Date(danes.toDateString())) {
      return 'preteklo'
    }

    // Prva 2 delovna dneva od danes sta blokirana
    ...
    while (blokiraniDatumi.length < 2) {
      ...
      if (dv !== 0 && dv !== 6) {
        blokiraniDatumi.push(
          pregledovanDan.toISOString().split('T')[0]
        )
      }

      pregledovanDan.setDate(
        pregledovanDan.getDate() + 1
      )
    }

    if (blokiraniDatumi.includes(dateStr)) {
      return 'ni_mozno'
    }

    // Admin blokade
    const celDanBlokiranAdmin =
      jeCelDanBlokiranAdmin(nedosegljivi, dateStr)

    const dopBlokiranAdmin =
      jeTerminBlokiranAdmin(
        nedosegljivi,
        dateStr,
        'dopoldne'
      )

    const popBlokiranAdmin =
      jeTerminBlokiranAdmin(
        nedosegljivi,
        dateStr,
        'popoldne'
    ...
    // Cel dan blokiran
    if (celDanBlokiranAdmin) {
      return 'ni_mozno'
    }

    const rezervacijeZaDan =
      rezervacije.filter(r => r.datum === dateStr)

    // Ni rezervacij in noben del dneva ni blokiran
    if (
    ...
    // Preverimo zasedenost posameznih delov dneva
    const zasedeneUre = rezervacijeZaDan
      .map(r => r.cas)
      .filter(Boolean)

    const imaDopoldne =
      zasedeneUre.includes('dopoldne') ||
      dopBlokiranAdmin

    const imaPopoldne =
      zasedeneUre.includes('popoldne') ||
      popBlokiranAdmin

    // Če sta oba dela dneva zasedena
    if (imaDopoldne && imaPopoldne) {
      return 'zasedeno'
    }

    // Vsaj en del dneva je še prost
    return 'prosto'
  }
  ...
  return (
    <div>
        ...
        >
          ...
          {Array(dniVMesecu)
            ...
            .map((_, i) => {
              ...
              const dateStr = `${leto}-${String(
                mesec + 1
              ).padStart(2, '0')}-${String(dan).padStart(
                2,
                '0'
              )}`
      ...
      >
        {[
          {
            barva: 'rgba(119,97,169,0.4)',
            label: 'Prosto',
          },
          {
            barva: 'rgba(180,60,60,0.4)',
            label: 'Zasedeno',
          },
          {
            barva: 'rgba(255,255,255,0.08)',
            label: 'Ni možno',
          },
        ].map(l => (
          <div
      ...
      >
        <div>
          ...
          <div
            style={{
              borderRadius: 10,
              ...errorStyle(showErrors, !velikost),
            }}
          >
            <Dropdown
              value={velikost}
        ...
        <div>
          ...
          <input
            ...
            placeholder="npr. zapestje, roka, gleženj..."
            style={{
              ...inputStyle,
              ...errorStyle(showErrors, !pozicija),
            }}
          />
        ...
        <div>
          ...
          <ErrorText
            show={showErrors && slike.length === 0}
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Stil, barve, posebne želje..."
            rows={3}
            style={{
              ...inputStyle,
              resize: 'vertical',
    ...
    <div>
      ...
      <div
        style={{
          ...
          gap: 14,
          borderRadius: 12,
          ...errorStyle(showErrors, !tipLaser),
        }}
      >
        {LASER_TIPI.map(t => (
          <div
      ...
      >
        {!jeHollywood && (
          <div>
            ...
            <input
              value={pozicija}
              onChange={e => setPozicija(e.target.value)}
              placeholder="npr. zapestje, obraz, roka..."
              style={{
                ...inputStyle,
                ...errorStyle(showErrors, !pozicija),
              }}
        ...
        <div>
          ...
          <textarea
            value={opombe}
            onChange={e => setOpombe(e.target.value)}
            placeholder="Barva, starost, posebnosti..."
            rows={3}
            style={{
              ...inputStyle,
              resize: 'vertical',
...
function Step4Odstr({
  ...
  const efektivnaVelikost =
    tipLaser === 'hollywood' || tipLaser === 'pege'
      ? 'do 5cm'
      : velikost
...
function StepPodatki({
  ...
  return (
    <div>
      ...
      >
        Za zagotovitev termina bo potreben avans v višini{' '}
        <strong style={{ color: '#fff' }}>
          {AVANS[storitev]} €
        </strong>
        , po potrditvi rezervacije.
      </p>
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              style={{
                ...inputStyle,
                ...errorStyle(showErrors, !priimek),
              }}
            />

            <ErrorText
              show={showErrors && !priimek}
            />
          </div>
        </div>
        ...
        <div>
          <label style={labelStyle}>
            ...
            >
              (neobvezno, le za lažjo komunikacijo)
            </span>
          </label>
...
export default function Booking() {
  ...
  const naloziRezervacije = async () => {
    const { data } = await supabase
      .from('rezervacije')
      .select(
        'datum, cas, storitev, velikost, tip_laser, status'
      )
  ...
  // Preverjanje termina tik pred oddajo.
  // VSE storitve uporabljajo enako logiko:
  // rezervacija zasede samo dopoldne ALI popoldne.
  const preveriCeJeTerminSeProst = () => {
    const rezervacijeZaDan =
      rezervacije.filter(r => r.datum === datum)

    // Cel dan blokiran s strani admina
    if (
      jeCelDanBlokiranAdmin(
        nedosegljivi,
        datum
      )
    ) {
    ...
    // Izbrani del dneva blokiran s strani admina
    if (
      jeTerminBlokiranAdmin(
        nedosegljivi,
        datum,
    ...
    // Preverimo, ali ima izbrani del dneva že rezervacijo
    const zasedeneUre =
      rezervacijeZaDan
        .map(r => r.cas)
        .filter(Boolean)

    if (zasedeneUre.includes(cas)) {
      return false
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await naloziNedosegljive()

      if (!preveriCeJeTerminSeProst()) {
        alert(
          'Ta termin se je medtem žal zapolnil. Prosim, izberi drug termin.'
        )
      ...
      for (const slika of slike) {
        ...
        const { error: uploadError } =
          await supabase.storage
            .from('rezervacije-slike')
            .upload(fileName, slika)
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              'radial-gradient(ellipse at center, rgba(119,97,169,0.2) 0%, transparent 70%)',
          }}
        />
      </div>
      ...
      >
        {!poslano ? (
          ...
          >
            ...
            >
              Tvojo rezervacijo bom
              pregledala v najkrajšem
              možnem času in te po emailu
              obvestila o{' '}
              <strong>
                potrditvi ali zavrnitvi
              </strong>{' '}
              termina.
              <br />
              <br />
              V primeru potrditve boš
              prejel/a tudi navodila za
            </p>
            ...
            >
              ki je obvezen za zagotovitev
              termina. V primeru zavrnitve
              pa si lahko poiščeš kakšen
              drug termin, ki bi ti ustrezal.
              <br />
            </p>
            ...
            >
              Zaprosila bi te še, da izpolniš
            </p>
