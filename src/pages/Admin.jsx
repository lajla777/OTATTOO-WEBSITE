...
export default function Admin() {
  ...
  const getPrikazStatusa = (r) => {
    if (r.status === 'potrjeno' && r.avans_placan) return 'nakazan avans'
    return r.status
  }

  const getStatusBarva = (r) => {
    if (r.status === 'potrjeno' && r.avans_placan) return 'avans'
    return r.status
  }
  ...
  const posodobiStatus = async (id, novStatus) => {
    ...
    if (novStatus === 'potrjeno') {
      updateData.avans_placan = false
      updateData.potrjeno_ob = new Date().toISOString()
    }

    if (novStatus === 'zavrnjeno') {
      updateData.avans_placan = false
    }

    if (novStatus === 'rezervirano') {
      updateData.avans_placan = false
      updateData.potrjeno_ob = null
    }
  ...
  const oznaciAvansPlacano = async (e, id) => {
    e.stopPropagation()

    if (!window.confirm('Ali je stranka res nakazala avans?')) return

    const { error } = await supabase
      .from('rezervacije')
      .update({ avans_placan: true })
      .eq('id', id)

    if (error) {
      alert('Napaka pri označevanju avansa.')
      return
    }
  ...
  const premakniVZavrnjenoBrezMejla = async (e, id) => {
  e.stopPropagation()

  if (!window.confirm('Ali želiš rezervacijo premakniti v zavrnjeno brez pošiljanja emaila?')) return
  ...
  if (error) {
    alert('Napaka pri premikanju rezervacije.')
    return
  }
  ...
  const getDanStyle = (status, jeIzbran) => {
    ...
    if (status === 'rezervirano') {
      return {
        ...base,
        background: 'rgba(230,160,30,0.2)',
  ...
  const getBadgeStyle = (r) => {
    ...
    if (status === 'avans') {
      return {
        background: 'rgba(119,97,169,0.22)',
        color: 'rgba(195,170,255,0.95)',
      }
    }
  ...
  const getCardBorder = (r) => {
    const status = getStatusBarva(r)

    if (status === 'rezervirano') return 'rgba(230,160,30,0.4)'
    if (status === 'potrjeno') return 'rgba(60,180,100,0.4)'
    if (status === 'avans') return 'rgba(119,97,169,0.55)'

    return 'rgba(180,60,60,0.3)'
  }

  const imeMeseca = new Date(leto, mesec, 1).toLocaleString('sl-SI', {
    month: 'long',
    year: 'numeric',
  })
  ...
  const filtrirane = rezervacije.filter(r => {
    if (filterStatus === 'vse') return true
    if (filterStatus === 'avans') return r.status === 'potrjeno' && r.avans_placan
    if (filterStatus === 'potrjeno') return r.status === 'potrjeno' && !r.avans_placan
    return r.status === filterStatus
  })

  const countFilter = (status) => {
    if (status === 'vse') return rezervacije.length
    if (status === 'avans') return rezervacije.filter(r => r.status === 'potrjeno' && r.avans_placan).length
    if (status === 'potrjeno') return rezervacije.filter(r => r.status === 'potrjeno' && !r.avans_placan).length
    return rezervacije.filter(r => r.status === status).length
  }
    ...
    border:
      barva === 'green'
        ...
        : barva === 'red'
          ...
          : barva === 'purple'
            ? '0.5px solid rgba(119,97,169,0.45)'
            : '0.5px solid rgba(230,160,30,0.4)',
    color:
      barva === 'green'
  ...
  return (
    <div style={{ display: 'flex', gap: 8 }}>
      {r.status === 'rezervirano' && (
        <>
          <button
            onClick={e => potrdiAkcijo(e, r.id, 'potrjeno', 'Ali si prepričana da želiš potrditi ta termin?')}
            style={{ ...btnStyle('green'), padding: p }}
          >
            ✓ Potrdi
          </button>

          <button
            onClick={e => potrdiAkcijo(e, r.id, 'zavrnjeno', 'Ali si prepričana da želiš zavrniti ta termin?')}
            style={{ ...btnStyle('red'), padding: p }}
          >
            ✕ Zavrni
          </button>
        </>
      ...
      {r.status === 'potrjeno' && !r.avans_placan && (
        <>
          <button
            onClick={e => oznaciAvansPlacano(e, r.id)}
            style={{ ...btnStyle('purple'), padding: p }}
          >
            ✓ Stranka je nakazala avans
          </button>

          <button
  onClick={e => premakniVZavrnjenoBrezMejla(e, r.id)}
  style={{ ...btnStyle('red'), padding: p }}
>
  🗑 Premakni v zavrnjeno
</button>
        </>
      )}
      ...
      {r.status === 'potrjeno' && r.avans_placan && (
        <>
          <div style={{
            ...
            textAlign: 'center',
            background: 'rgba(119,97,169,0.22)',
            border: '0.5px solid rgba(119,97,169,0.45)',
            color: 'rgba(195,170,255,0.95)',
          }}>
            ✓ Avans nakazan
          </div>
...
>
  🗑 Premakni v zavrnjeno
      ...
      {r.status === 'zavrnjeno' && (
        <div style={{ display: 'flex', gap: 8, flex: 1 }}>
          <button
            onClick={e => potrdiAkcijo(e, r.id, 'rezervirano', 'Ali si prepričana da želiš obnoviti ta termin?')}
            style={{ ...btnStyle('yellow'), padding: p }}
          >
            ↩ Obnovi
          </button>

          <button
            onClick={e => izbrisiRezervacijo(e, r.id)}
            style={{ ...btnStyle('red'), padding: p }}
          >
            🗑 Izbriši
          </button>
  ...
  const PodrobnostiRezervacije = ({ r }) => (
    <>
      <div style={{
        display: 'flex',
        ...
        {r.potrjeno_ob && r.status === 'potrjeno' && !r.avans_placan && (
          <p style={{ fontSize: 12, color: 'rgba(255,200,80,0.8)', margin: '4px 0 0' }}>
            ⏳ Čaka na avans od {new Date(r.potrjeno_ob).toLocaleDateString('sl-SI')}
          </p>
        )}

        {r.avans_placan && (
          <p style={{ fontSize: 12, color: 'rgba(195,170,255,0.95)', margin: '4px 0 0' }}>
            ✓ Avans nakazan
          </p>
        )}
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <button
            onClick={() => geslo === ADMIN_GESLO && setPrijavljen(true)}
            style={{
              width: '100%',
              padding: 14,
              borderRadius: 50,
  ...
  return (
    <div style={{
      ...
      minHeight: '100vh',
      color: '#fff',
      padding: isMobile ? '90px 12px 60px' : '100px 24px 60px',
    }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {vCakanju > 0 && (
              <button
                onClick={() => {
                  setPogled('rezervacije')
                  setFilterStatus('rezervirano')
                }}
                style={{
                  padding: '8px 16px',
                  borderRadius: 50,
                  ...
                  cursor: 'pointer',
                  background: 'rgba(230,160,30,0.2)',
                  border: '0.5px solid rgba(230,160,30,0.4)',
                  color: 'rgba(255,200,80,0.9)',
                }}
              >
                ⏳ {vCakanju} v čakanju
                ...
                {Array(dniVMesecu).fill(null).map((_, i) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (status !== 'vikend') {
                          setIzbranDatum(dateStr)
                          setIzbranRezervacija(null)
                        }
                      }}
                      style={getDanStyle(status, jeIzbran)}
                    >
                      {dan}
                    </div>
                  )
            ...
            <div style={{
              ...
              {!izbranDatum ? (
                <p style={{
                  ...
                  Klikni na datum za podrobnosti
                </p>
              ) : (
                <div>
                  <h3 style={{
                  ...
                  {rezervacijeNaDatum.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {rezervacijeNaDatum.map(r => (
                        <div
                          key={r.id}
                          onClick={() => setIzbranRezervacija(izbranRezervacija?.id === r.id ? null : r)}
                          style={{
                            padding: 16,
                            borderRadius: 12,
                            cursor: 'pointer',
                        ...
                        >
                          ...
                          <p style={{
                            ...
                            {r.storitev}{r.tip_laser ? ` · ${r.tip_laser}` : ''}
                          </p>
                ...
                {filtrirane.map(r => (
                  <div
                    ...
                    style={{
                      ...cardStyle,
                      padding: 20,
                      border: `0.5px solid ${getCardBorder(r)}`,
                    }}
                  >
                    <div style={{
                      display: 'flex',
